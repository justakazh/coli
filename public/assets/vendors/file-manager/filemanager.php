...
$auth_file_manager_user = env('FILE_MANAGER_USER');
$auth_file_manager_pass = env('FILE_MANAGER_PASS');
$auth_users = array(
    $auth_file_manager_user => password_hash($auth_file_manager_pass, PASSWORD_DEFAULT)
);
...
if ($use_auth) {
    //custom bypass login after user authenticated in apps
    if(isset($_GET['hash'])){
        $cred = md5(env("FILE_MANAGER_USER").":".env("FILE_MANAGER_PASS"));
        if($_GET['hash'] == $cred){
            $_SESSION[FM_SESSION_ID]['logged'] = env("FILE_MANAGER_USER");
        }
        else{
